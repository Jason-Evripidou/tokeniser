...
public:
    ...
    inline void printWordCounts(const std::string& filename) const
    {
        ...
        for(const auto& entry : m_word_counts)
        {
            const bpe::Byte* word = entry.first;
            const std::size_t count = entry.second.getCount();

            output_file << reinterpret_cast<const char*>(word) << " : " << count << '\n';
        }
    }
    //---------------------------------------------------------------------------------------//
};
